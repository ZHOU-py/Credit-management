...
class NN(object):
    ...
    def __init__(self, sizes, opts, X, Y):
        ...
        #这里的size三次循环，分别初始化三层NN，前两层为RBM，最后一层为输出为10的输出层
        for size in self._sizes + [Y.shape[1]]:
            #这里maxrange为 4*sqrt(6/(20+400)) 和 4*sqrt(6/(400+100)) 和 4*sqrt(6/(100+10))
            max_range = 4 * math.sqrt(6. / (input_size + size))
            #这里初始化的参数w是根据上面的界限值来随即初始化的
            self.w_list.append(
                np.random.uniform(
    ...
    def load_from_dbn(self, dbn):
        ...
        """
        #这个函数负责载入已经训练好的w和b参数
        # Cette fonction est responsable du chargement des paramètres w et b qui ont été formés
    ...
    def train(self):
        ...
        """
        # Cette fonction pour calculer parametre w et b dans le  Neural Network.
        ...
        with tf.Session() as sess:
            ...
            for i in range(self._opts._epoches):
                ...
                #print (np.mean(np.argmax(self._Y, axis=1) == sess.run(predict_op, feed_dict={
